...
This is ScalaZ3 for Z3 4.8.14 and Scala 2.13.
...
to compile Z3 4.8.14 and compile ScalaZ3 for Scala 2.13.

The JAR files will be in `target/scala-2.13/scalaz3_2.13-4.8.14.jar`
...
The JAR files will be in `target/scala-2.13/scalaz3_2.13-4.8.14.jar` and will contain the shared library
dependencies.
...
Create a folder named `unmanaged` at the same level as your `build.sbt` file, and copy the JAR file in `target/scala-2.13/scalaz3_2.13-4.8.14.jar` into it.

Then add, the following lines to your `build.sbt` file:

```scala
Compile / unmanagedJars += {
  baseDirectory.value / "unmanaged" / s"scalaz3_2.13-4.8.14.jar"
}
```
...
Make sure to name the resulting JAR files as `scalaz3-[osName]-[osArch]-2.13.jar`, where:
...
Compile / unmanagedJars += {
  baseDirectory.value / "unmanaged" / s"scalaz3-$osName-$osArch-2.13.jar"
}
```
